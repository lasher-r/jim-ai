        ...
        try:
            while True:
                ...
                if mode == "motion":
                    ...
                    # run YOLO/HOG less often for speed
                    run_now = (frame_i % (skip+1) == 0)

                    if run_now:
                        try:
                            # If cv2.dnn exists and model files are present, use YOLO
                            dets, classes = yolo_detect(frame, conf_th=conf_th, nms_th=nms_th, size=yolo_size)
                            # keep only 'person'
                            for cid, conf, (x,y,wc,hc) in dets:
                                lbl = classes[cid] if 0 <= cid < len(classes) else str(cid)
                                if lbl == "person":
                                    hits.append((x,y,wc,hc, conf, lbl))
                            last_dets, last_classes = dets, classes
                        except Exception:
                            # Fallback: HOG people detector (no dnn needed)
                            dets = hog_person_detect(frame)  # -> list of (label, conf, (x,y,w,h))
                            hits = [(x,y,wc,hc,float(conf),"person") for (label,conf,(x,y,wc,hc)) in dets]
                            last_dets = [(-1, float(conf), (x,y,wc,hc)) for (_,conf,(x,y,wc,hc)) in dets]
                            last_classes = ["person"]
                    else:
                        # reuse last detections to keep it smooth between inference frames
                        dets = last_dets if last_dets else []
                        classes = last_classes if last_classes else ["person"]
                        for item in (dets or []):
                            cid, conf, (x,y,wc,hc) = item
                            lbl = (classes[cid] if (cid >= 0 and classes and 0 <= cid < len(classes)) else "person")
                            if lbl == "person":
                                hits.append((x,y,wc,hc, (None if cid < 0 else conf), lbl))
